...
Then, for the depth descriptors (HON), we defined $\theta_{I} = 8$ and $\phi_{G} = 8$, whereas for the thermal descriptors (HIOG), we defined $T_{I} = 8$ and $T_{G} = 8$. A very large number of bins is not convenient for generalization. Besides, we wanted the number of bins describing orientations in the different descriptions to coincide for comparison purposes in the later experiments; and 8 bins for describing normal vectors' orientations seem a quite reasonable amount of bins in view of the depth sensor's noisy measurements.

Other parameters have to be empirically selected, as the number of mixtures $k$ for GMMs, the $\tau$ parameters, or the corresponding classifiers' parameters in the learning-based fusion approaches. The number of mixtures $k$ tried $\{2,3, ..., 9, 11, 13\}$, whereas $\tau$ takes $32$ values in the range $(-2,2)$.
...
to the background model. Another issue is that some regions considered as unknown Ð
mostly those generated when one person overlaps other Ð considerably differ from those
