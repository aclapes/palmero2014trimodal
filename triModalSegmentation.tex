...
\begin{figure}[ht!]
	\centering
	\includegraphics[width=\linewidth]{pictures/diagram.eps}
...
%The main idea of the grid partitioning is to reduce the variability of the regions in each GMM. At the same time, they are large enough to not condition the eventually computed overlap measure.
%our purpose was to have a low dimensional - lower than the original - yet descriptive feature vector, so
...
%Those are quite large feature vectors i per aixo applicat PCA 95\% info . goal: que els feature vectors fueran de una largaria parecida. tab para evitar curse of dimensionality. 
...
%It is therefore a pessimistic measure because a very tiny blob misclassified as a person in the predicted binary masks will account for 0 overlap, thus decreasing the mean overlap of the frame, so it can be considered as a lower bound on how accurate the prediction is.
...
%\begin{figure*}[htbp]
%	\centering
%		\includegraphics[width=0.4\textwidth]{individual_overlap.eps} \\
%		\vspace{0.5cm}
%		\includegraphics[width=0.4\textwidth]{simple_fusion_cd_overlap.eps}
%		\hspace{0.1cm}
%		\includegraphics[width=0.4\textwidth]{simple_fusion_t_overlap.eps} \\
%		\vspace{0.5cm}
%		\includegraphics[width=0.4\textwidth]{learning_cd_overlap.eps} 
%		\hspace{0.1cm}
%		\includegraphics[width=0.4\textwidth]{learning_t_overlap.eps}
%	\caption{Overlap results without mirroring.}
%	\label{fig:overlap_no_mirroring}
%\end{figure*}
%
%\begin{figure*}[htbp]
%	\centering
%		\includegraphics[width=0.4\textwidth]{individual_mirroroverlap.eps} \\
%		\vspace{0.5cm}
%		\includegraphics[width=0.4\textwidth]{simple_fusion_cd_mirroroverlap.eps}
%		\hspace{0.1cm}
%		\includegraphics[width=0.4\textwidth]{simple_fusion_t_mirroroverlap.eps} \\
%		\vspace{0.5cm}
%		\includegraphics[width=0.4\textwidth]{learning_cd_mirroroverlap.eps} 
%		\hspace{0.1cm}
%		\includegraphics[width=0.4\textwidth]{learning_t_mirroroverlap.eps}
%	\caption{Overlap results with mirroring.}
%	\label{fig:overlap_mirroring}
%\end{figure*}
%
%\begin{figure}[htbp]
%	\centering
%		\includegraphics[width=0.5\textwidth]{individual_mirroring.eps} \\
%		\vspace{0.1cm}
%		\includegraphics[width=0.5\textwidth]{simple_fusion_mirroring.eps} \\
%		\vspace{0.1cm}
%		\includegraphics[width=0.5\textwidth]{learning_fusion_mirroring.eps} 
%	\caption{Mirroring vs. non-mirroring comparison.}
...
%overlap discussion:
The obtained results show that fusing different descriptions enhances the representation of the scene, thus increasing the final overlap when segmenting subjects and discriminating from other artifacts present in the scene. However, the selection of the fusion approach is crucial. Our proposed heuristic naive approach for fusing individual confidences of each descriptor decreases the overlap of the depth descriptor - the one that gives the best result among individual predictions - up to 4\%, but substantially improves the results if we compare them to the other modalities. On the other hand, as observed by the learning-based fusion experiments, we obtain significant performance improvements regarding each individual modality and the heuristic fusion strategy. In contrast to the subject detection experiment, we achieve the best results using SVM with RBF kernel, which outperforms the motion descriptor by 6\%.

It is important to note that thermal descriptions cannot reach as good overlap values as the other descriptions owing to the fact that the binary masks $F^{thermal}$ were created from $F^{depth}$ using the registration algorithm, which cannot be accurate up to pixel level, in such a way that the ground-truth and registered masks will moderately differ, especially in left and right sides of the image. Therefore, we cannot conclude whether the proposed thermal description performs accurately.
...
Finally, the comparison between overlap results with and without mirroring shows that the mirrored versions of our proposed predictions show a slight improvement with respect to the non-mirrored versions. Such improvement is more noticeable in the simple fusion approaches, in which the overlap increases up to 2\%. 
